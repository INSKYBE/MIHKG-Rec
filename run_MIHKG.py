...
from utils.parser import parse_args_MIHKG
from utils.data_loader import load_data
from modules.MIHKG_Rec import MIHKG
from utils.evaluate_MIHKG import test
...
try:
    ...
    # Try importing the C++ sampler for faster execution
    path = join(dirname(__file__), "utils/ext/sampling.cpp")
    sampling = imp_from_filepath(path)
    sampling.seed(seed)
except:
    # Fallback to Python's uniform sampler if C++ extension is not available
    sampling = UniformSampler(seed)

# Set the process title for monitoring purposes
setproctitle.setproctitle('EXP@KGRec')


def neg_sampling_cpp(train_cf_pairs, train_user_dict):
    # Function to perform negative sampling using C++ module
    time1 = time()
...
if __name__ == '__main__':
    try:
        ...
        # Set intervals for testing and early stopping criteria
        test_interval = 10 if args.dataset == 'last-fm' else 1
        early_stop_step = 5 if args.dataset == 'last-fm' else 10

        # Variables for tracking the best performance and early stopping
        cur_best_pre_0 = 0
        cur_stopping_step = 0
        should_stop = False
        ...
        for epoch in range(args.epoch):
            ...
            with Progress() as progress:
                task = progress.add_task("[cyan]Processing...", total=len(train_cf) // args.batch_size)
            ...
            if epoch % test_interval == 0 and epoch >= 1:
                ...
                model.eval()
                with torch.no_grad():
                    ret = test(model, user_dict, n_params,args)
                test_e_t = time()
                ...
                # Use rich to create a visually appealing table
                table = Table(title="Epoch Results")
                table.add_column("Epoch", justify="center", style="cyan")
                table.add_column("Training Time", justify="center")
                table.add_column("Testing Time", justify="center")
                table.add_column("Loss", justify="center")
                table.add_column("Recall", justify="center")
                table.add_column("NDCG", justify="center")
                ...
                table.add_row(
                    ...
                    ", ".join([f"{v:.4f}" for v in add_loss_dict.values()]),
                    ", ".join([f"{v:.4f}" for v in ret['recall']]),
                    ", ".join([f"{v:.4f}" for v in ret['ndcg']]),
                    ", ".join([f"{v:.4f}" for v in ret['precision']]),
                    ", ".join([f"{v:.4f}" for v in ret['hit_ratio']])
                )
                console.print(table)
                # *********************************************************
