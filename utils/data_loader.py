...
def read_triplets(file_name):
    ...
    if args.inverse_r:
        ...
        inv_triplets_np[:, 1] += max(can_triplets_np[:, 1]) + 1
        can_triplets_np[:, 1] += 1
        inv_triplets_np[:, 1] += 1
...
def build_graph(train_data, triplets):
    ...
    cprint("Begin to load interaction triples ...", "yellow", attrs=['bold'])
    for u_id, i_id in tqdm(train_data, ascii=True):
        rd[0].append([u_id, i_id])

    cprint("\nBegin to load knowledge graph triples ...", "yellow", attrs=['bold'])
...
def build_sparse_relational_graph(relation_dict):
    ...
    adj_mat_list = []
    cprint("Begin to build sparse relation matrix ...", "yellow", attrs=['bold'])
...
def load_data(model_args):
    ...
    cprint('reading train and test user-item set ...', 'green')
    train_cf = read_cf(directory + 'train.txt')
    test_cf = read_cf(directory + 'test.txt')
    cprint('interaction count: train %d, test %d' % (train_cf.shape[0], test_cf.shape[0]), 'green')
    remap_item(train_cf, test_cf)

    cprint('combinating train_cf and kg data ...', 'green')
    triplets = read_triplets(directory + 'kg_final.txt')

    cprint('building the graph ...', 'green')
    graph, relation_dict = build_graph(train_cf, triplets)

    cprint('building the adj mat ...', 'green')
